...
const sendEmail = (data) => {
  ...
  const { req, res, token, user, confirm } = data;
  ...
  readHTMLFile('./api/emails/emailTemplate.html', function(err, html) {
    ...
    transporter.sendMail(mailOptions, function(err) {
      if (err) {
        // htmlstream.close();
        return res.status(500).json({ info: 'Something went wrong', err });
      }
      res.status(200).json({ info: 'An email has been sent to ' + user.email, token});
    });
  });
};
...
module.exports = function(app) {
  app.post('/signup', auth.optional, (req, res, next) => {
    ...
    User.find({ email: newUser.email }, function(err, user) {
      if (err) {
        return res.status(404).json({ info: 'Cannot check if user exists' })
      }
      if (user.length <= 0) {
        ...
        finalUser.save().then(() => {
          ...
          var token = new Token({ _userId: finalUser._id });
          token.generateToken();
        ...
        return res.status(404).json({ info: 'The email address you have entered is already associated with another account.' })
      }
    });
  });
  ...
  app.post('/confirmation/:token', auth.optional, (req, res) => {
    Token.findOne({ token: req.params.token }, function (err, token) {
      if(err || !token) {
        res.status(400).json({ info: 'We were unable to find a valid token. Your token my have expired.', errors: err });
      }
      if(token) {
        User.findOne({ _id: token._userId }, function(err, user){
          if (!user) return res.status(400).json({ info: 'We were unable to find a user for this token.' });
          if (user.isVerified) return res.status(400).json({ info: 'This user has already been verified.' });

          user.isVerified = true;
          user.save(function(err) {
            if(err) {
              return res.status(500).json({ info: 'Unable to activate user', errors: err });
            }
            return res.status(200).json({ info: 'Account succesfully activated'});
          });
        });
      }
    });
  ...
  app.post('/resend', auth.optional, (req, res) => {
    User.findOne({ email: req.body.email }, function (err, user) {
      ...
      var token = new Token({ _userId: user._id });
      token.generateToken();
  ...
  app.post('/reset/:token', auth.optional, (req, res) => {
    User.findOne({ resetPasswordToken: req.params.token}, function(err, user) {
      ...
      user.save(function(err) {
        if(err) return res.status(500).json({ errors: 'Unable to change password' });
        return res.status(200).json({ info: 'Password changed successfully' })
      });
    });
  });
  ...
  app.post('/login', auth.optional, (req, res, next) => {
    ...
    return passport.authenticate('local', { session: false }, (err, passportUser, info) => {
      ...
      if(passportUser) {
        ...
        if(!user.isVerified) {
          return res.status(400).json({info: 'You need to activate account to LogIn' })
        }
  ...
  app.put('/users/:id', auth.required, (req, res) => {
    User.findById(req.params.id, function(err, user) {
      if(err) {
        return res.status(404).json({ error: 'Cannot perform user update' });
      }
      if(user) {
        ...
        user.save(function(err, user) {
          if(err) {
            return res.status(404).json({ errors: 'Error when saving user' });
          }

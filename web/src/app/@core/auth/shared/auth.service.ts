...
export class AuthService {
  ...
  activateAccount(token: string): Observable<boolean> {
    return this.httpClient.post(`${this.baseApiUrl}/confirmation/${token}`, {}, {
      observe: 'response',
    }).pipe(
  ...
  resendToken(email: string): Observable<boolean> {
    return this.httpClient.post(`${this.baseApiUrl}/resend`, { email }, {
      observe: 'response',
    }).pipe(
  ...
  sendResetToken(email: string): Observable<boolean> {
    return this.httpClient.post(`${this.baseApiUrl}/forgot`, { email }, {
      observe: 'response',
    }).pipe(
      map((response: HttpResponse<Object>) => response.ok),
      catchError(err => of(false))
  ...
  changePass({ pass }, token): Observable<boolean> {
    return this.httpClient.post(`${this.baseApiUrl}/reset/${token}`, {pass}, {
      observe: 'response',
    }).pipe(
      map((response: HttpResponse<Object>) => response.ok),
      catchError(err => of(false)),
